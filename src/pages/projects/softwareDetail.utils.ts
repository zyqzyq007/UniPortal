...
export const filterTreeNodes = (nodes: FileTreeNode[], keyword: string): FileTreeNode[] => {
...
export const getLanguageByPath = (filePath: string) => {
...
export const getThemeBySystem = (isDark: boolean) => (isDark ? 'vs-dark' : 'vs')

export const isBinaryPath = (filePath: string) => {
  const ext = filePath.split('.').pop()?.toLowerCase() || ''
